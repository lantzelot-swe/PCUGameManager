...
public class ScreenshotsPanel extends JPanel
{
	...
	private String editTooltip = "<html>Optimal resolution for the carousel is 320x200.<br>Press to edit the image.<br>It will be resized when the game is saved but it might make it blurry.</html>";
	private boolean gamesFileUpdated = false;

	FileNameExtensionFilter imagefilter = new FileNameExtensionFilter("png, gif, jpeg, bmp", "png", "gif", "jpg", "jpeg",
			"bmp");
	...
	private void reloadScreens()
	{
		...
		} else
		{
			...
			{
				currentScreen1Image = loadScreen(modelScreen1File, getScreen1ImageLabel(), getResolution1Label(),
						getEdit1Button());
		...
		} else
		{
			...
			{
				currentScreen2Image = loadScreen(modelScreen2File, getScreen2ImageLabel(), getResolution2Label(),
						getEdit2Button());
	...
	private void selectGameFile()
	{
		...
		FileNameExtensionFilter vicefilter = new FileNameExtensionFilter("Vice runnable files", "d64", "t64", "tap", "VSF",
				"GZ", "crt", "prg", "g64");
	...
	private void selectCoverFile()
	{
		...
		if (value == JFileChooser.APPROVE_OPTION)
		{
			...
			model.setCoverImage(handleCoverFileDrop(new File[]
			{ selectedFile }, coverImageLabel));
		}
	}
	...
	private void selectScreenshotFile(boolean first)
	{
		...
		if (value == JFileChooser.APPROVE_OPTION)
		{
			...
			if (first)
			{
				model.setScreen1Image(handleScreenFileDrop(new File[]
				{ selectedFile }, screen1ImageLabel, edit1Button));
			} else
			{
				model.setScreen2Image(handleScreenFileDrop(new File[]
				{ selectedFile }, screen2ImageLabel, edit2Button));
			}
		}
	}
